...
// Referer/Origin (которые требует kinobox) теперь ставит бэкенд-прокси server-side —
// браузер запрещает их выставлять из JS («Refused to set unsafe header»).
// timeout — чтобы недоступный kinobox не вешал загрузку плеера, а быстро падал
// в fallback на kinobd.
...
// Не показываем трейлеры с ютуба и Netflix (требует аккаунт)
...
const toPlayersMap = (providers = [], { type = null, translationId = null } = {}) => {
  ...
  for (const provider of providers) {
    ...
    if (isUnwantedPlayer(`${providerType} ${provider?.iframeUrl || ''}`)) {
      continue
    }
