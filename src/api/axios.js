...
const getResolvedBaseUrl = async () => {
  ...
  const stored = apiStore.currentApiUrl
  // Уважаем только относительный сохранённый URL (наш прокси /api-backend/...).
  // Старые абсолютные значения (https://api4.rhserv.vu) из localStorage игнорируем —
  // иначе они перебивали бы прокси и возвращали CORS/rate-limit.
...
const attachDynamicRequestState = (instance) => {
  ...
  instance.interceptors.response.use(
    (res) => res,
    (err) => {
      // Логируем на debug, а не error: почти все вызовы rhserv имеют fallback
      // (kinobd), и при rate-limit это ожидаемый шум, а не настоящая ошибка.
      const status = err?.response?.status
      console.debug('[api] request failed', status, err?.config?.url)
