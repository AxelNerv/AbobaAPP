...
const FALLBACK_DESCRIPTION =
  'AbobaTv - online movie and TV streaming with collections, ratings, and simple navigation.'
const SITE_NAME = 'AbobaTv'
...
// Сид-каталог (src/data/movies.json, ~300 записей) больше НЕ импортируется сюда
// статически. Раньше он вкомпилировался в стартовый чанк и весил 118 КБ gzip —
// каждый посетитель качал и парсил справочник, который нужен только для
// SEO-подстраховки. Теперь его подгружает main.js на этапе SSG-сборки
// (includedRoutes), а в браузере карта наполняется из ответов API.
const moviesByKpId = new Map()

// Вызывается только при пререндере (Node). В браузер этот путь не идёт.
export const loadSeedMovieSeoEntries = (rawMovies = []) => {
  if (!Array.isArray(rawMovies)) return []
  const normalized = rawMovies.map(normalizeMovie).filter(Boolean)
  for (const movie of normalized) moviesByKpId.set(movie.kp_id, movie)
  return normalized
}
...
// Список для пререндера собирается из того, что загрузил loadSeedMovieSeoEntries.
export const getPrerenderMovieSeoEntries = () => Array.from(moviesByKpId.values())
