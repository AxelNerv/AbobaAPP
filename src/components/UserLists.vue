  ...
  <div class="wrapper">
    <div class="user-lists-page" tabindex="0">
      <div class="controls">
        <div class="filter-card type-card">
          <div class="button-group type-buttons">
            <AppIcon name="movie" :size="20" class="card-icon" />
            <button
            ...
            >
              <AppIcon :name="isImporting ? 'waiting' : 'upload'" :size="18" />
            </button>
            <button
      ...
      <div v-if="movies.length === 0 && !loading" class="empty-state">
        <AppIcon name="movie" :size="48" :stroke-width="1.5" />
...
const downloadJson = (payload) => {
  ...
  anchor.href = url
  anchor.download = `abobatv-export-${date}.json`
