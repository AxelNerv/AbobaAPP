      ...
      try {
        ...
        try {
          for (const title of [...mainStore.history].reverse()) {
            // Передаём полный объект title — он уже содержит kp_id, name_ru/title,
            // poster, year. Без этого на сервере останутся только пустые kp_id.
            await addToList(title.kp_id, USER_LIST_TYPES_ENUM.HISTORY, title)
          }
